...
void pickLegalKaidan(vector<Yaku> *legalYaku, const vector<Yaku> &allYaku, const Table &table){
    ...
    if(table.isKakumei()){//革命のとき
        for(i=0;i<allYaku.size();i++){
            ...
            if( allYaku[i].isKaidan() && 
                (allYaku[i].mNum==table.mNum && allYaku[i].mRankR<table.mRankL)
                ){
        ...
        for(i=0;i<allYaku.size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( (allYaku[i].isKaidan()) &&
                 allYaku[i].mNum==table.mNum && allYaku[i].mRankL>table.mRankR)
                {
                if( !table.isShibari() || (table.isShibari() && table.mSuits==allYaku[i].mSuits)){
                    legalYaku->push_back( allYaku[i] );
                }
...
void pickLegalPair(vector<Yaku> *legalYaku, const vector<Yaku> &allYaku, const Table &table){
    ...
    if(table.isKakumei()){//革命のとき
        for(i=0;i<allYaku.size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( allYaku[i].isPair() && 
               allYaku[i].mNum==table.mNum && allYaku[i].mRankL<table.mRankL
               ){
                if( !table.isShibari() || (table.isShibari() && table.mSuits==allYaku[i].mSuits)){
                    legalYaku->push_back( allYaku[i] );
                }
...
void pickLegalTanki(vector<Yaku> *legalYaku, const vector<Yaku> &allYaku, const Table &table){
    ...
    if(table.isJTanki()){//ジョーカー単騎はスペ３しか考えられない
        ...
        for(i=0; i < allYaku.size(); i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if(allYaku[i].mNum==1 && allYaku[i].mRankL<table.mRankL){
        ...
        for(i=0; i < allYaku.size(); i++){
            ...
            if(allYaku[i].mNum==1 && allYaku[i].mRankR>table.mRankL){
                if( !table.isShibari() || (table.isShibari() && table.mSuits==allYaku[i].mSuits || allYaku[i].isJTanki() )){
                   legalYaku->push_back( allYaku[i] );
                }
            }
				...
				if(searching==1 && length>=3){//階段が作れる
					...
					if(joker_flag==1){//jokerを使わずに来ていれば、jokerに置き換えた階段も作ることができる
						for(i=rank;i<rank+length;i++){
							...
							Yaku cd(hdj, suit, length, rank, rank+length-1, JposSuit, JposRank);
							yaku->push_back(cd);
					    }
					}
				}
				...
				if(joker==1){//jokerに置き換えたバージョンを（8で使うくらいしか利点がわからない）
					...
					for(suit=0;suit<4;suit++){//jokerに置き換えるスート
						if(PATTERN[pat][suit]==1){
							...
							Yaku cd(hdj, bitSuit, num, rank, rank, JposSuit, JposRank);
				            yaku->push_back(cd);
						}
					}
				}
...
void sortKaidan(vector<Yaku> *tky, const vector<Yaku> *atky, const Table &table){
    if(table.isKakumei()){//革命のとき
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( (*atky)[i].isKaidan() && 
                ((*atky)[i].mNum==table.mNum && (*atky)[i].mRankR<table.mRankL)
                ){
        ...
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( ((*atky)[i].isKaidan()) &&
                 (*atky)[i].mNum==table.mNum && (*atky)[i].mRankL>table.mRankR)
                {
                if( !table.isShibari() || (table.isShibari() && table.mSuits==(*atky)[i].mSuits)){
                    tky->push_back((*atky)[i]);
                }
...
void sortPair(vector<Yaku> *tky, const vector<Yaku> *atky, const Table &table){
    if(table.isKakumei()){//革命のとき
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( ((*atky)[i].isPair()) && 
               (*atky)[i].mNum==table.mNum && (*atky)[i].mRankL<table.mRankL
               ){
                if( !table.isShibari() || (table.isShibari() && table.mSuits==(*atky)[i].mSuits)){
                    tky->push_back((*atky)[i]);
                }
        ...
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if( ((*atky)[i].isPair()) && 
                (*atky)[i].mNum==table.mNum && (*atky)[i].mRankR>table.mRankR
                ){
                if( !table.isShibari() || (table.isShibari() && table.mSuits==(*atky)[i].mSuits)){
                    tky->push_back((*atky)[i]);
                }
...
void sortTanki(vector<Yaku> *tky, const vector<Yaku> *atky, const Table &table){
    ...
    if(table.isJTanki()){//ジョーカー単騎はスペ３しか考えられない
        ...
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if((*atky)[i].mNum==1 && (*atky)[i].mRankL<table.mRankL){
        ...
        for(int i=0;i<atky->size();i++){
            //階段で枚数が同じでかつ左側の強さが場札の強さより大きい
            if((*atky)[i].mNum==1 && (*atky)[i].mRankR>table.mRankR){
                if( !table.isShibari() || (table.isShibari() && table.mSuits==(*atky)[i].mSuits || (*atky)[i].isJTanki() )){
                    tky->push_back((*atky)[i]);
                }
            }else if((*atky)[i].mNum==0){//パスは残す
